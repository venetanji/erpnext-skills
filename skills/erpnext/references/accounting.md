...
14. **Multi-currency JE — `exchange_rate == 1` is treated as "unset" and refetched.** *Symptom:*
...
15. **Bank Transaction reconciles via its child table after submit** — it allows
...
16. **Importing a Bank Transaction is staging, not posting** — it does **not** move the GL; the
...
17. **An Account with cancelled GL entries can't be deleted.** *Symptom:*
